---
title: "Configuration"
permalink: /mmistakes/configuration/
...
Consult the [JavaScript documentation]({{ site.baseurl }}{% link _mmistakes/17-javascript.md %}) for more information on working with theme scripts.
{: .notice--info}
...
Breadcrumb start link text and separator character can both be changed in the [UI Text data file]({{ "/mmistakes/ui-text/" | relative_url }}).
...
To customise the separator between the page date and reading time (if both are enabled), edit `.page__meta-sep::before` in a [custom stylesheet]({{ "/mmistakes/stylesheets/" | relative_url }}).
